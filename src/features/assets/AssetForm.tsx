...
import { centsToEuroInput, euroInputToCentsSafe } from '@shared/utils/money';
...
export function AssetForm({ initialData, onSubmit, onCancel }: AssetFormProps) {
  ...
  const validate = () => {
    ...
    if (formData.monthlyContribution && (isNaN(Number(formData.monthlyContribution)) || parseFloat(formData.monthlyContribution) < 0)) {
      newErrors.monthlyContribution = 'Sparbeitrag muss >= 0 sein';
    }
    setErrors(newErrors);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Input
          label={`Kostenbasis (${getCurrencySymbol()})`}
          type="number"
          step="0.01"
          required
      ...
      <div className="grid grid-cols-2 gap-3">
        <Input
          label={`Marktwert (${getCurrencySymbol()})`}
          type="number"
          step="0.01"
      ...
      <Textarea
        ...
        placeholder="z.B. ISIN, Broker, etc."
        rows={2}
      />
